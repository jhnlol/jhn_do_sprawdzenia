...
local token = "MTEyNjc4NDg4MTQ2OTQzMTg4OQ.GFEB_4.hsNYCKdRUumFfcGA_DTp9ucaFXy6Tao9RLcs6c"
local prefix = "!"
...
function ExecuteCOMM(command)
    if string.starts(command, prefix) then

    if string.starts(command, prefix .. "do_sprawdzenia") then

            local t = mysplit(command, " ")

            if t[2] ~= nil then
                sendToDiscord("Hounds system ",
                              "Pomyslnie dodano dcid " .. t[2],
                              16711680)
                              
                Main.dodawanie(t[2], nil)
            else 
                sendToDiscord("JHN",
                "Nie podales dcid",
                16711680)
